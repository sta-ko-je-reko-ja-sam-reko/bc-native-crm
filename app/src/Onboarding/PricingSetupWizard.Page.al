...
/// </summary>
page 50192 "NBC Pricing Setup Wizard"
{
    PageType = NavigatePage;
    ApplicationArea = All;
    Caption = 'Pricing Flexibility — Setup';
    SourceTable = "NBC Pricing Setup";
    InsertAllowed = false;
    DeleteAllowed = false;
    ...
    var
        ...
        OverviewLbl: Label 'Pricing Flexibility lets you choose a pricing method per sales line and reuse discount tiers across customers, so the right price and break applies automatically. This guide enables the feature and, optionally, loads sample data.';
        EnableLbl: Label 'Turn the feature on. Enabling it shows the per-line pricing method, the reusable discount tiers and the related actions. The session restarts when you finish so the change takes effect.';
        SampleLbl: Label 'Importing sample data adds ready-made example records for this feature to the CURRENT company so you can explore and test it right away — for Pricing: example discount lists with quantity-break tiers covering the different discount and pricing-method options. It is safe: the data uses fixed sample IDs and importing never creates duplicates, so you can run it again with no harm. It builds on the standard demo company''s customers and salespeople; on an empty company it simply loads what it can. This is example data for evaluation — review it before relying on it in a company you use for real work. Leave this off if you only want to enable the feature.';
