...
/// </summary>
page 50185 "NBC Activities Setup Wizard"
{
    PageType = NavigatePage;
    ApplicationArea = All;
    Caption = 'Activities and Timeline — Setup';
    ...
    var
        ...
        OverviewLbl: Label 'Activities and Timeline lets you log every interaction — tasks, calls, appointments, emails and notes — and read them all on one timeline. This guide enables the feature and, optionally, loads sample data.';
        EnableLbl: Label 'Turn the feature on. Enabling it shows the timeline and the activity actions. The session restarts when you finish so the change takes effect.';
        SampleLbl: Label 'Importing sample data adds ready-made example records for this feature to the CURRENT company so you can explore and test it right away — for Activities: a spread of example activities — tasks, phone calls, appointments, emails and notes in different statuses and priorities — logged against demo customers and contacts. It is safe: the data uses fixed sample IDs and importing never creates duplicates, so you can run it again with no harm. It builds on the standard demo company''s customers and salespeople; on an empty company it simply loads what it can. This is example data for evaluation — review it before relying on it in a company you use for real work. Leave this off if you only want to enable the feature.';
