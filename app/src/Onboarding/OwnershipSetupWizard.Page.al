...
/// </summary>
page 50184 "NBC Ownership Setup Wizard"
{
    PageType = NavigatePage;
    ApplicationArea = All;
    Caption = 'Ownership and Teams — Setup';
    ...
    var
        ...
        OverviewLbl: Label 'Ownership and Teams gives every customer, contact and opportunity an owner — a salesperson or a team — so you can see who is responsible and focus on your own records. This guide enables the feature and, optionally, loads sample data.';
        EnableLbl: Label 'Turn the feature on. Enabling it shows the owner fields, the Teams pages and the related actions. The session restarts when you finish so the change takes effect.';
        SampleLbl: Label 'Importing sample data adds ready-made example records for this feature to the CURRENT company so you can explore and test it right away — for Ownership: a couple of demo teams with members and example record owners. It is safe: the data uses fixed sample IDs and importing never creates duplicates, so you can run it again with no harm. It builds on the standard demo company''s customers and salespeople; on an empty company it simply loads what it can. This is example data for evaluation — review it before relying on it in a company you use for real work. Leave this off if you only want to enable the feature.';
