...
/// </summary>
page 50191 "NBC Catalog Setup Wizard"
{
    PageType = NavigatePage;
    ApplicationArea = All;
    Caption = 'Product Sales Catalog — Setup';
    SourceTable = "NBC Catalog Setup";
    InsertAllowed = false;
    DeleteAllowed = false;
    ...
    var
        ...
        OverviewLbl: Label 'The Product Sales Catalog turns your items and resources into things you can sell as a set — product bundles and kits with component lines, cross-sell and up-sell relationships between products, and a selling lifecycle status on each item and resource. This guide enables the feature and, optionally, loads sample data.';
        EnableLbl: Label 'Turn the feature on. Enabling it shows the product bundle and kit pages, the cross-sell and up-sell relationship pages, the catalog status fields and the related actions. The session restarts when you finish so the change takes effect.';
        SampleLbl: Label 'Importing sample data adds ready-made example records for this feature to the CURRENT company so you can explore and test it right away — for the Product Sales Catalog: example product bundles with component lines, cross- and up-sell relations, and catalog status set on several demo items and resources. It is safe: the data uses fixed sample IDs and importing never creates duplicates, so you can run it again with no harm. It builds on the standard demo company''s items and resources; on an empty company it simply loads what it can. This is example data for evaluation — review it before relying on it in a company you use for real work. Leave this off if you only want to enable the feature.';
