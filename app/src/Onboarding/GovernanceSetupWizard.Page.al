...
/// </summary>
page 50190 "NBC Governance Setup Wizard"
{
    PageType = NavigatePage;
    ApplicationArea = All;
    Caption = 'Governance — Setup';
    SourceTable = "NBC Governance Setup";
    InsertAllowed = false;
    DeleteAllowed = false;
    ...
    var
        ...
        OverviewLbl: Label 'Governance gives your CRM data audit logging and duplicate detection so you can track who changed what and catch records that clash before they cause trouble. This guide enables the feature and, optionally, loads sample data.';
        EnableLbl: Label 'Turn the feature on. Enabling it shows the audit logging and duplicate-detection behaviour and the related actions. The session restarts when you finish so the change takes effect.';
        SampleLbl: Label 'Importing sample data adds ready-made example records for this feature to the CURRENT company so you can explore and test it right away — for Governance: two example customers that deliberately share the same name, so the duplicate-detection feature has something to find. It is safe: the data uses fixed sample IDs and importing never creates duplicates, so you can run it again with no harm. It builds on the standard demo company''s customers and salespeople; on an empty company it simply loads what it can. This is example data for evaluation — review it before relying on it in a company you use for real work. Leave this off if you only want to enable the feature.';
