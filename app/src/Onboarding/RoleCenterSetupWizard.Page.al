...
/// </summary>
page 50189 "NBC Role Center Setup Wizard"
{
    PageType = NavigatePage;
    ApplicationArea = All;
    Caption = 'CRM Role Center — Setup';
    ...
    var
        ...
        OverviewLbl: Label 'The CRM Role Center is a salesperson home page with cue tiles and shortcuts for your CRM work. This guide enables the feature and, optionally, loads sample data.';
        EnableLbl: Label 'Turn the feature on. Enabling it shows the CRM Role Center home page with its cue tiles and shortcuts. The session restarts when you finish so the change takes effect.';
        SampleLbl: Label 'Importing sample data adds ready-made example records for this feature to the CURRENT company so you can explore and test it right away — for the Role Center: it links your user to a salesperson so the home-page cue tiles show figures instead of being empty. It is safe: it changes only your own User Setup link, so you can run it again with no harm. It builds on the standard demo company''s customers and salespeople; on an empty company it simply loads what it can. This is example data for evaluation — review it before relying on it in a company you use for real work. Leave this off if you only want to enable the feature.';
