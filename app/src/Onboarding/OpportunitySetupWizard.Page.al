...
/// </summary>
page 50187 "NBC Opportunity Setup Wizard"
{
    PageType = NavigatePage;
    ApplicationArea = All;
    Caption = 'Opportunity Depth — Setup';
    ...
    var
        ...
        OverviewLbl: Label 'Opportunity Depth turns a basic opportunity into a full deal — product and resource lines, competitors, stakeholders, a rating and a rolled-up estimated value. This guide enables the feature and, optionally, loads sample data.';
        EnableLbl: Label 'Turn the feature on. Enabling it shows the deal fields, the product lines, competitors and stakeholders pages and the related actions. The session restarts when you finish so the change takes effect.';
        SampleLbl: Label 'Importing sample data adds ready-made example records for this feature to the CURRENT company so you can explore and test it right away — for Opportunity Depth: a few example opportunities complete with product and resource lines, competitors, stakeholders and a rolled-up estimated value, based on demo contacts and items. It is safe: the data uses fixed sample IDs and importing never creates duplicates, so you can run it again with no harm. It builds on the standard demo company''s customers and salespeople; on an empty company it simply loads what it can. This is example data for evaluation — review it before relying on it in a company you use for real work. Leave this off if you only want to enable the feature.';
